...
else Some1_vt{stream_vt(a)}(xs) // [else]
//
) (* end of [lam] *)
//
in
...
| ~stream_vt_nil() => let
    val () =
      xs0 := $ldelay(stream_vt_nil) in None1_vt()
    // end of [val]
  end // end of [stream_vt_nil]
...
| ~stream_vt_nil
    () => None1_vt((*void*))
| ~stream_vt_cons
    (x0, xs) => $effmask_all(Some1_vt(loop(x0, xs)))
//
...
(
  case+ !xs of
  | ~stream_vt_nil() => None1_vt()
  | ~stream_vt_cons(x, xs) =>
      if n = 0 then (~xs; Some1_vt(x)) else loop(xs, pred(n))
) (* end of [loop] *)
//
...
| @stream_vt_cons(x, xs) => let
  ...
  in
    case+ oy of
    | ~None1_vt() => auxmain_con(xs)
    | ~Some1_vt(y) => stream_vt_cons{b}(y, auxmain(xs))
  end (* end of [stream_vt_con] *)
//
end // end of [let]
//
...
in
  case+ opt of
  | ~None1_vt() => stream_vt_nil()
  | ~Some1_vt(x0) => stream_vt_cons{a}(x0, aux(st))
end // end of [let]
)
//
...
| ~stream_vt_nil
    ((*void*)) => stream_vt_nil()
| ~stream_vt_cons(x, xs) =>
    !(auxmain2(x, xs, $UN.cast{List_1(y)}(ys0)))
)
,
(
  lazy_vt_free(xs);
  List_vt_free<y>($UN.castvwtp0{List_vt_1(y)}(ys0))
)
) (* end of [auxmain] *)
//
and
...
,
(~xs; List_vt_free<y>($UN.castvwtp0{List_vt_1(y)}(ys0)))
) (* end of [auxmain2] *)
//
in
  auxmain(xs0)
...
implement
{}(*tmp*)
streamer_vt_make(xs) = STREAMER(Some1_vt(xs))
...
| @Some1_vt(xs) =>
  (
    ...
    | ~stream_vt_nil() =>
      (
        free@(opt);
        opt := None1_vt(); fold@(xser); None1_vt()
      )
    | ~stream_vt_cons(x0, xs1) =>
      (
        xs := xs1; fold@(opt); fold@(xser); Some1_vt(x0)
      )
  ) (* end of [Some_vt] *)
